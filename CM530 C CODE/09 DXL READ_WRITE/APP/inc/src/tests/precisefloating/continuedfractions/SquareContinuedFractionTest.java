 ...
 */
public class SquareContinuedFractionTest extends TestCase {

    Rational r;
    RationalExpansion re;
    PartialQuotients pqi;
    Rational convergent;
    ContinuedFraction square;

    private static final Logger log = Logger.getLogger(SquareContinuedFractionTest.class.getName());

    public void testIntegers() {
        r = Rational.ZERO;
        re = new RationalExpansion(r);
        square = re.square();
        pqi = square.partialQuotients();
        ContinuedFractionTestUtils.assertStartsWith(new long[]{0}, false, pqi);
        Convergents convergents = square.convergents();
    ...
    public void testNegativeIntegers() {
        ...
        ContinuedFractionTestUtils.assertLastConvergentEquals(r.square(), square.convergents());
    ...
    public void testSquareSpeed() {
        Random rnd = new Random(/*0*/);
        Monitor squareMon, multiplyMon;

        squareMon = multiplyMon = null;

        for (int i = 0; i < 7; i++) {
            r = new Rational(1000, rnd);
            re = new RationalExpansion(r);

            square = re.square();
            squareMon = MonitorFactory.start("rational square");
            ContinuedFractionTestUtils.assertLastConvergentEquals(r.square(), square.convergents());
            squareMon.stop();

            square = re.multiply(re);
            multiplyMon = MonitorFactory.start("rational square by multiplication");
            ContinuedFractionTestUtils.assertLastConvergentEquals(r.square(), square.convergents());
            multiplyMon.stop();
        }

        log.info("rational square " + squareMon.toString());
        log.info("rational square by multiplication " + multiplyMon.toString());
        ...
        for (int i = 0; i < 7; i++) {
            square = ContinuedFractionConstants.E.square();
            squareMon = MonitorFactory.start("E square");
            ContinuedFractionTestUtils.assertStartsWith(expected, true, square.partialQuotients());
            squareMon.stop();

            square = ContinuedFractionConstants.E.multiply(ContinuedFractionConstants.E);
            multiplyMon = MonitorFactory.start("E square by multiplication");
            ContinuedFractionTestUtils.assertStartsWith(expected, true, square.partialQuotients());
            multiplyMon.stop();
        }

        log.info("e square " + squareMon.toString());
        log.info("e square by multiplication " + multiplyMon.toString());
        ...
        for (int i = 0; i < 7; i++) {
            square = ContinuedFractionConstants.GoldenRatio.square();
            squareMon = MonitorFactory.start("GoldenRatio square");
            ContinuedFractionTestUtils.assertStartsWith(expected, true, square.partialQuotients());
            squareMon.stop();

            square = ContinuedFractionConstants.GoldenRatio.multiply(
                    ContinuedFractionConstants.GoldenRatio);
            multiplyMon = MonitorFactory.start("GoldenRatio square by multiplication");
            ContinuedFractionTestUtils.assertStartsWith(expected, true, square.partialQuotients());
            multiplyMon.stop();
        }

        log.info("GoldenRatio square " + squareMon.toString());
        log.info("GoldenRatio square by multiplication " + multiplyMon.toString());
    }
